...
void loop() {
  ...
  if (irValue > FINGER_ON ) {
    ...
    totalBPM += avgBeat;
    if(spO2 < 90.0) totalSpO2 += 90.0;
    else totalSpO2 += spO2;
    numReadings++;	
      
    Serial.print("totalTemperature: " + String(totalTemperature));
    Serial.print(", totalBpm:" + String(totalBPM));
    Serial.print(", totalSpO2: " + String(totalSpO2));
  ...
  if (currentTime - startTime >= 60000) {
    ...
    float avgBPM = totalBPM / numReadings;
    double avgSpO2 = totalSpO2 / numReadings;
...
double calculateSpO2(uint32_t ir, uint32_t red) {
  if (i % Num_Readings == 0) {
    if (aveir > 0 && avered > 0 && sumirrms > 0 && sumredrms > 0) {
      double R = (sqrt(sumirrms) / aveir) / (sqrt(sumredrms) / avered);
      SpO2 = -23.3 * (R - 0.4) + 100;
    } else {
      SpO2 = 0; // Giá trị mặc định khi tính toán không hợp lệ
    }
